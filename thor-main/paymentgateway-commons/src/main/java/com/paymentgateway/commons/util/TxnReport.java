...
public class TxnReport {
	...
	private static Logger logger = LoggerFactory.getLogger(TxnReport.class.getName());
	...
	public List<TransactionSearch> searchPayment(String pgRefNum, String orderId, String customerEmail, String SKUCode,
			...
			Set<String> orderIdSet, boolean allowLatest, String transactionFlag, String deltaFlag) {
		Map<String, User> userMap = new HashMap<String, User>();

		logger.info("Inside TxnReports , searchPayment");
		boolean isGlocal = false;

		UserSettingData sessionUserSettings = userSettingDao.fetchDataUsingPayId(user.getPayId());
		UserSettingData merchantSettings = null;
		...
		try {
			...
			if (!merchantPayId.equalsIgnoreCase("ALL")) {
				...
				merchantSettings = userSettingDao.fetchDataUsingPayId(merchantPayId);
			}
			...
			while (cursor.hasNext()) {
				...
				if (merchantPayId.equalsIgnoreCase("All")) {
					...
					if (user.getUserType().equals(UserType.ADMIN) || user.getUserType().equals(UserType.SUBADMIN)
						...
						if (sessionUserSettings.isRetailMerchantFlag()) {
							if (StringUtils.isNotBlank(dbobj.getString(FieldType.REFUND_DAYS.getName()))) {
								transReport.setRefundCycle(dbobj.getString(FieldType.REFUND_DAYS.getName()));
							} else {
								transReport.setRefundCycle(CrmFieldConstants.NA.getValue());
...
//					}
				...
				if (((user.getUserType().equals(UserType.MERCHANT)) || (user.getUserType().equals(UserType.SUBUSER)))
						&& sessionUserSettings.isCustomTransactionStatus()) {
					if (((dbobj.getString(FieldType.STATUS.toString())).equalsIgnoreCase(StatusType.TIMEOUT.getName()))
	...
	public int searchPaymentCount(String pgRefNum, String orderId, String customerEmail, String SKUCode,
			...
			boolean allowLatest, String transactionFlag, String deltaFlag) {
	...
	public int transactionCount(String pgRefNum, String orderId, String customerEmail, String merchantPayId,
			...
			Set<String> orderIdSet, String transactionFlag, String deltaFlag) {
	...
	public List<TransactionSearch> transactionReport(String pgRefNum, String orderId, String customerEmail,
		...
		UserSettingData sessionUserSettings = userSettingDao.fetchDataUsingPayId(sessionUser.getPayId());
			...
			while (cursor.hasNext()) {
				...
				if (merchantPayId.equalsIgnoreCase("All")) {
					...
					if (sessionUser.getUserType().equals(UserType.ADMIN)
						...
						User merhant = userMap.get(merchantPayId); // userdao.findPayId(payid);
						UserSettingData merchntSettings = userSettingDao.fetchDataUsingPayId(merhant.getPayId());

						if (merchntSettings.isRetailMerchantFlag()) {
							if (StringUtils.isNotBlank(dbobj.getString(FieldType.REFUND_DAYS.getName()))) {
								transReport.setRefundCycle(dbobj.getString(FieldType.REFUND_DAYS.getName()));
							} else {
								transReport.setRefundCycle(CrmFieldConstants.NA.getValue());
...
//					}
	...
	public List<TransactionSearch> refundForSaleCaputureTransaction(List<TransactionSearch> transList) {
		...
		try {
			...
			logger.error("Exception caught in Txn Reports getForRefundTransaction() ", e);
		}
		return transactionSearchList;
	}
	...
	public List<MerchantPaymentAdviseDownloadObject> merchantPaymentAdviceDownloadForSale(String merchantPayId,
			String subMerchantPayId, String payoutDate, User user, Set<String> orderIdSet) {
		...
		try {
			...
			while (cursor.hasNext()) {
				...
				merchantPaymentReport.setCreateDate(dbobj.getString(FieldType.CREATE_DATE.getName()));
	...
	public List<TransactionSearch> getBillingDetails(List<TransactionSearch> transList) {
		...
		try {
			...
			logger.error("Exception caught in TxnReports getBillingDetails() ", ex);
		}
		return transList;
	}
...
//			if (StringUtils.isNotBlank(custId) && !custId.equalsIgnoreCase("ALL")) {
//				paramConditionLst.add(new BasicDBObject(FieldType.CUST_ID.getName(), custId));
//			}
...
//			User user = new User();
//
//			if (StringUtils.isNotBlank((String) documentObj.get(FieldType.PAY_ID.getName()))) {
...
//	}
	...
	public List<TransactionSearch> deliveryStatusForSaleCaputureTransaction(List<TransactionSearch> transList) {
		...
		try {
			...
			logger.error("Exception caught in Txn Reports delivery status for sale capture report ", e);
		}
		return transactionSearchList;
	}
	...
	public List<TransactionSearch> getDispatchDetails(List<TransactionSearch> transList) {
		...
		try {
			...
			logger.error("Exception caught in TxnReports getDispatchDetails ", ex);
		}
		return transList;
	}
...
//				}
				...
				if (user.getUserType().equals(UserType.ADMIN) || user.getUserType().equals(UserType.SUBADMIN)) {
					...
					if (StringUtils.isNotBlank(dbobj.getString(FieldType.CUSTOMER_ACCOUNT_NO.toString()))) {
						eCollectionList
								.setpaymentGatewayAccountNumber(dbobj.getString(FieldType.CUSTOMER_ACCOUNT_NO.toString()));
					} else {
...
//						}
	...
	public List<VendorPayouts> billingDetailsForVendorPayoutTransactions(List<VendorPayouts> vendorPayoutList) {
		...
		try {
			...
			logger.error("Exception caught in TxnReports billingDetailsForVendorPayoutTransactions function ", ex);
		}
		return vendorPayoutList;
	}
...
//				}
				...
				/*
				 * if
				 * (StringUtils.isNotBlank(dbobj.getString(FieldType.TXN_CAPTURE_FLAG.getName())
	...
	public List<TransactionSearch> fraudAnalyticsReportData(String merchantPayId, String subMerchantPayId,
		...
		try {
			...
			if (!countryCode.equalsIgnoreCase("ALL") && StringUtils.isNotBlank(countryCode)) {
				isParameterised = true;
				paramConditionLst.add(new BasicDBObject(FieldType.COUNTRY.getName(), countryCode));
			}

			if (!status.equalsIgnoreCase("ALL") && StringUtils.isNotBlank(status)) {
				isParameterised = true;
				paramConditionLst.add(new BasicDBObject(FieldType.STATUS.getName(), status));
			}
			...
			while (cursor.hasNext()) {
				Document dbobj = cursor.next();
				logger.info("fatch data from cursor1");
				orderId.add(dbobj.getString(FieldType.ORDER_ID.getName()));
			}
			cursor.close();
			for (String id : orderId) {
				// List<BasicDBObject> fianlListt = new ArrayList<BasicDBObject>();
				BasicDBObject finalquery2 = new BasicDBObject(FieldType.ORDER_ID.getName(), id);
				/*
				 * if (!allParamQuery.isEmpty()) { fianlListt.add(allParamQuery); } if
				 * (!finalquery2.isEmpty()) { fianlListt.add(finalquery2); }
				 */
				// BasicDBObject finalqueryy = new BasicDBObject("$and", fianlListt);
				...
				while (cursor2.hasNext()) {
					...
					if (StringUtils.isNotBlank(paymentRegion) && !paymentRegion.equalsIgnoreCase("ALL")) {
						if (!(paymentRegion.equalsIgnoreCase(dbobj.getString(FieldType.PAYMENTS_REGION.getName())))) {
							break;
						}
					}
					if (StringUtils.isNotBlank(countryCode) && !countryCode.equalsIgnoreCase("ALL")) {
						if (!(countryCode.equalsIgnoreCase(dbobj.getString(FieldType.COUNTRY.getName())))) {
							break;
						}
					}
...
//		if (payId.equals(PropertiesManager.propertiesMap.get("MSEDCL_PAY_ID"))) {
...
//			statusQuery = new BasicDBObject(FieldType.STATUS.getName(), "Settled");
//			txntypeQuery = new BasicDBObject(FieldType.TXNTYPE.getName(), "RECO");
//		}
