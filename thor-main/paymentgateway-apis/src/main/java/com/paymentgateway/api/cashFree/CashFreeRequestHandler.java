...
@Service
public class CashFreeRequestHandler {

	private static Logger logger = LoggerFactory.getLogger(CashFreeRequestHandler.class.getName());
	...
	@Autowired
	private CashFreeResponseHandler cashFreeResponseHandler;
	...
	public static String TO_GENTERATE_NEW_TOKEN = PropertiesManager.propertiesMap.get("CASHFREE_TO_GENTERATE_NEW_TOKEN");
	public static String TO_GENTERATE_VIRTUAL_ACCOUNT = PropertiesManager.propertiesMap.get("CASHFREE_TO_GENTERATE_VIRTUAL_ACCOUNT");
	public static String TO_GENTERATE_VPA = PropertiesManager.propertiesMap.get("CASHFREE_TO_GENTERATE_VPA");
	public static String TO_GET_QR_CODE_FOR_EXISTING = PropertiesManager.propertiesMap.get("CASHFREE_TO_GET_QR_CODE_FOR_EXISTING");
	...
	public static String requestCreaterToken(String request, String vpa, Fields fields)throws SystemException {
		String hostUrl = "";
		String responseData = "";
		try {
			String requestTYpe = "POST";
		...
		public Fields genrateToken(Fields fields) {
			...
			try {
					...
					respObj.put("clientId", CASHFREE_CLIENT_ID);
					respObj.put("clientSecret", CASHFREE_CLIENT_SECRET);
					fields.put(FieldType.REQUEST_TYPE.getName(),"TO_GENTERATE_NEW_TOKEN");
					String response =requestCreaterToken(respObj.toString(), null, fields);
		...
		public Map<String, String>  genrateVirtualAccountRequest(Fields fields) {
			...
			try {
					respObj = new JSONObject();
					respObj.put("vAccountId", fields.get(FieldType.VIRTUAL_ACC_NUM.getName()));
					...
					//if(fields.get(FieldType.RESPONSE_CODE.getName())!=null && fields.get(FieldType.RESPONSE_CODE.getName()).equalsIgnoreCase("000")) {
						//cashFreeDBEntry.addNewEntryData(fields, "virtualAccount");
					//}
		...
		public Map<String, String> genrateVPARequst(Fields fields) {
			...
			try {
					respObj = new JSONObject();
					respObj.put("virtualVpaId", fields.get(FieldType.VIRTUAL_VPA_NUM.getName()));
					respObj.put("name", fields.get(FieldType.CUST_NAME.getName()));
					respObj.put("phone", fields.get(FieldType.CUST_PHONE.getName()));
					respObj.put("email", fields.get(FieldType.CUST_EMAIL.getName()));
					respObj.put("notifGroup", "DEFAULT");
					...
					//if(fields.get(FieldType.RESPONSE_CODE.getName())!=null && fields.get(FieldType.RESPONSE_CODE.getName()).equalsIgnoreCase("000")) {
						//cashFreeDBEntry.updateEntryData(fields, "virtualVPA");
		...
		public Map<String, String> genrateCreateQRCodeRequst(Fields fields) {
			...
			try {
					...
					//if(fields.get(FieldType.RESPONSE_CODE.getName())!=null && fields.get(FieldType.RESPONSE_CODE.getName()).equalsIgnoreCase("000")) {
						//cashFreeDBEntry.updateQrCode(fields,"virtualQR", fields.get(FieldType.VPA.getName()));
					//}
		...
		public Map<String, String> genrateCashFreeQRCode(Fields fields) {
			...
			try {
				...
				switch (reqid) {
				case 1:
					saveResponse=genrateVirtualAccountRequest(fields);
					Fields responseData = new Fields(saveResponse);
					if(fields.get(FieldType.RESPONSE_CODE.getName())!=null && fields.get(FieldType.RESPONSE_CODE.getName()).equalsIgnoreCase("000")) {
						cashFreeDBEntry.addNewEntryData(fields, "virtualAccount");
						saveResponse.clear();
						fields.put(FieldType.VIRTUAL_VPA_NUM.getName(),TransactionManager.getNewTransactionId());
						saveResponse=genrateVPARequst(fields);
						responseData = new Fields(saveResponse);
						if(fields.get(FieldType.RESPONSE_CODE.getName())!=null && fields.get(FieldType.RESPONSE_CODE.getName()).equalsIgnoreCase("000")) {
							cashFreeDBEntry.updateDataEntry(fields, "virtualVPA",fields.get(FieldType.VIRTUAL_ACC_NUM.getName()));
							saveResponse.clear();
							saveResponse=genrateCreateQRCodeRequst(fields);
							responseData = new Fields(saveResponse);
							if(fields.get(FieldType.RESPONSE_CODE.getName())!=null && fields.get(FieldType.RESPONSE_CODE.getName()).equalsIgnoreCase("000")) {
								cashFreeDBEntry.updateDataEntry(fields,"virtualQR", fields.get(FieldType.VIRTUAL_ACC_NUM.getName()));
							}
						}
					}
					break;
		...
		public void federalResponseHandler(HttpServletRequest httpRequest, HttpServletResponse httpResponse)
			...
			try {
				...
				}else {fields.put(FieldType.PAY_ID.getName(), null);} 
		...
		public void vaUpdateStatusRequest(Fields fields) {
