...
@Service("federalProcessor")
public class FederalProcessor implements Processor {

	@Autowired
	private FederalIntegrator federalIntegrator;

	@Autowired
	private FederalUpiIntegrator federalUpiIntegrator;
	...
	public void process(Fields fields) throws SystemException {
		...
		if ((fields.get(FieldType.TXNTYPE.getName()).equals(TransactionType.NEWORDER.getName())
			...
			// New Order Transactions are not processed by Federal
			return;
		}

		if (!fields.get(FieldType.ACQUIRER_TYPE.getName()).equals(AcquirerType.FEDERAL.getCode())) {
			return;
		}
		...
		if (paymentType.equals(PaymentType.UPI.getCode())) {
			...
			federalUpiIntegrator.process(fields);
		} else {

			federalIntegrator.process(fields);
		}
	}
