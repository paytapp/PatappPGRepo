...
@Service("p2PTSPProcessor")
public class P2PTSPProcessor implements Processor{

	@Autowired
	private P2PTSPIntegrator p2PTSPIntegrator;
	@Override
	...
	public void process(Fields fields) throws SystemException {
		...
		if ((fields.get(FieldType.TXNTYPE.getName()).equals(TransactionType.NEWORDER.getName())
			...
			// New Order Transactions are not processed by SabPaisa
			return;
		}

		if (!fields.get(FieldType.ACQUIRER_TYPE.getName()).equals(AcquirerType.P2PTSP.getCode())) {
			return;
		}
		// if the MOP Type is GPay
		String mopType = fields.get(FieldType.MOP_TYPE.getName());

		if (mopType.equals(MopType.QR.getCode())) {
			try {
				p2PTSPIntegrator.process(fields);
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
