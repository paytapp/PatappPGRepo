...
public enum SafexpayMopType {
	...
	private SafexpayMopType(String bankName, String code, String bankCode) {
	...
	public static SafexpayMopType getInstance(String name) {
		SafexpayMopType[] mopTypes = SafexpayMopType.values();
		for (SafexpayMopType mopType : mopTypes) {
	...
	public static List<SafexpayMopType> getGetMopsFromSystemProp(String mopsList) {

		List<String> mopStringList = (List<String>) Helper.parseFields(PropertiesManager.propertiesMap.get(mopsList));

		List<SafexpayMopType> mops = new ArrayList<SafexpayMopType>();

		for (String mopCode : mopStringList) {
			SafexpayMopType mop = getmop(mopCode);
			mops.add(mop);
		}
		return mops;
	}
	...
	public static String getmopName(String mopCode) {
		SafexpayMopType mopType = SafexpayMopType.getmop(mopCode);
		if (mopType == null) {
			return "";
		}
	...
	public static String getBankCode(String code) {
		SafexpayMopType mopType = SafexpayMopType.getmop(code);
		if (mopType == null) {
			return "";
		}
	...
	public static SafexpayMopType getmop(String mopCode) {
		SafexpayMopType mopObj = null;
		if (null != mopCode) {
			for (SafexpayMopType mop : SafexpayMopType.values()) {
	...
	public static SafexpayMopType getInstanceIgnoreCase(String name) {
		SafexpayMopType[] mopTypes = SafexpayMopType.values();
		for (SafexpayMopType mopType : mopTypes) {
