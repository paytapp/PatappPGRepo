...
@Service
public class SafexpayStatusEnquiryProcessor {
	...
	private static Logger logger = LoggerFactory.getLogger(SafexpayStatusEnquiryProcessor.class.getName());
	...
	public void enquiryProcessor(Fields fields) {
		...
		try {
			...
			logger.error("Exception in decrypting status enquiry response for safexpay ", e);
		}
	...
	public static String getResponse(String request) throws SystemException {
		...
		try {
			...
			hostUrl = PropertiesManager.propertiesMap.get(Constants.SAFEXPAY_STATUS_ENQ_URL);
			...
			throw new SystemException(ErrorType.INTERNAL_SYSTEM_ERROR, ioException,
					"Network Exception with Safexpay for status enquiry request " + hostUrl.toString());
		}
	...
	public void updateFields(Fields fields, String response) {
		...
		if (fields.get(FieldType.INTERNAL_ORIG_TXN_TYPE.getName()).equalsIgnoreCase(TransactionType.SALE.getCode())) {
			...
			if ((StringUtils.isNotBlank(transaction.getRes_code())) && (StringUtils.isNotBlank(transaction.getStatus()))
					&& ((transaction.getRes_code()).equalsIgnoreCase(Constants.STATUS_ENQ_SUCCESS_CODE)) 
...

			{
				...
				if (StringUtils.isNotBlank(transaction.getRes_message())) {
					pgTxnMsg = transaction.getRes_message();
				}
				...
				if ((StringUtils.isNotBlank(transaction.getRes_code()))) {
					...
					if (StringUtils.isNotBlank(transaction.getRes_code())) {
						respCode = transaction.getRes_code();
					}

					SafexpayResultType resultInstance = SafexpayResultType.getInstanceFromName(respCode);
	...
	public Transaction toTransaction(String jsonResponse, String txnType, String order) {
		...
		try {
			...
			if (StringUtils.isBlank(jsonResponse)) {
				...
				logger.info("Empty response received for safexpay status enquiry request");
				return transaction;
			}
