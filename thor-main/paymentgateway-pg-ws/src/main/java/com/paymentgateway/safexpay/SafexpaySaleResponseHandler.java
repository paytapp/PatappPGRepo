...
@Service
public class SafexpaySaleResponseHandler {
	private static Logger logger = LoggerFactory.getLogger(SafexpaySaleResponseHandler.class.getName());
	...
	public Map<String, String> process(Fields fields) throws SystemException {
		...
		String response = fields.get(FieldType.SAFEXPAY_RESPONSE_FIELD.getName());
		...
		safexpayTransformer.updateResponse(fields);

		fields.put(FieldType.INTERNAL_ORIG_TXN_TYPE.getName(), fields.get(FieldType.TXNTYPE.getName()));
		fields.remove(FieldType.SAFEXPAY_RESPONSE_FIELD.getName());
	...
	public Transaction toTransaction(String response, Fields fields) {
		...
		if (StringUtils.isBlank(response)) {
			logger.info("Empty response received from Safex Pay");
			return transaction;
		}
