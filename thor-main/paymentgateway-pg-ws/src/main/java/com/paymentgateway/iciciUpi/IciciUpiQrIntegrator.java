...
@Service
public class IciciUpiQrIntegrator {
	...
	private static final Logger logger = LoggerFactory.getLogger(IciciUpiQrIntegrator.class.getName());
...
//		qrObjList.add(new BasicDBObject(FieldType.STATUS.getName(), StatusType.SENT_TO_BANK.getName()));
	...
	private String generateIntentCallString(Fields fields) throws SystemException {
		logger.info("Generating Intent Call String for ICICI UPI QR Code");
		try {
			...
			intentCallString.append(fields.get(FieldType.ADF2.getName()));
			intentCallString.append(Constants.AMPERSAND);
			intentCallString.append(Constants.PN);
			intentCallString.append(Constants.EQUATOR);
			intentCallString.append(fields.get(FieldType.ADF3.getName()));
			intentCallString.append(Constants.AMPERSAND);
			intentCallString.append(Constants.TR);
			intentCallString.append(Constants.EQUATOR);
			intentCallString.append(PropertiesManager.propertiesMap.get(Constants.UPI_QR_PREFIX));
			...
			intentCallString.append(Constants.AMPERSAND);
			intentCallString.append(Constants.MC);
			intentCallString.append(Constants.EQUATOR);
			intentCallString.append(fields.get(FieldType.TXN_KEY.getName()));
