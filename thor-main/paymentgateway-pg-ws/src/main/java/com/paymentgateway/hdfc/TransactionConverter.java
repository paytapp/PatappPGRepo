...
public class TransactionConverter {
	...
	public String statusEnquiryRequest(Fields fields, Transaction transaction) {
		...
		getElement(AMT, amount, xml);
		getElement(TRANSID, fields.get(FieldType.ORIG_TXN_ID.getName()), xml);
		getElement(TRACKID, fields.get(FieldType.ORIG_TXN_ID.getName()), xml);
		getElement(UDF5, udf5, xml);
		 getElement(MEMBER, Constants.MEMBER, xml); 
		xml.append(REQUEST_CLOSE_TAG);
		return xml.toString();
	...
	public void logRequest(String requestMessage, Fields fields){
		log("Request message to" +fields.get(FieldType.ACQUIRER_TYPE.getName()) +": TxnType = " + fields.get(FieldType.TXNTYPE.getName()) + " " + "Txn id = " + fields.get(FieldType.TXN_ID.getName()) + " " + "Url= "+ requestMessage, fields);
	}
