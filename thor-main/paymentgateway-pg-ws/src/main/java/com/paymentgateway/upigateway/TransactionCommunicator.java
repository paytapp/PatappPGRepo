 ...
 *
 */
@Service("upigatewayTransactionCommunicator")
public class TransactionCommunicator {
	...
	public void updateSaleResponse(Fields fields, String paymentUrl) {
		...
		fields.put(FieldType.UPIGATEWAY_FINAL_REQUEST.getName(), paymentUrl);
	...
	public String refundPostRequest(String request, String hostUrl) throws SystemException {
		...
		// Refund is not supported by UPIGateway
		String response = "";
		return response;
	}
	...
	public String statusEnqPostRequest(String hostUrl,String request,Transaction transaction) throws SystemException {
		...
		try {
			...
			logger.info("UPIGateway Status Enquiry request = {}", request);
			...
			logger.info("Response received for UPIGateway Status Enquiry response {} ", response.toString());
	...
	public String getPaymentResponse(String request, Fields fields, Transaction transaction) throws SystemException {
		...
		try {
			...
			StringBuilder response = new StringBuilder();
			String hostUrl = PropertiesManager.propertiesMap.get(Constants.UPIGATEWAY_SALE_URL);

			logger.info("UPIGateway Sale request = {}", request);
			...
			logger.info("Response received for UPIGateway sale URL request {} ", response.toString());
			return response.toString();

		} catch (Exception e) {
			logger.error("Exception in getting Sale URL Response for UPIGateway", e);
		}
		return stringResponse;
	}
