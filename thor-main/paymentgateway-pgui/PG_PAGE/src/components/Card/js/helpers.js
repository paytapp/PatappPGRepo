...
const helpers = {
    ...
    cardValidationHandler: function(inputLength) {
        if (inputLength < 9) {
            window.id('emptyCardNumber').style.display = "none";
            window.id('emiBinCheckError').style.display = "none";
    ...
    emiValidationHandler: function(inputLength) {
        if (inputLength < 6) {
            window.id('emptyCardNumber').style.display = "none";
            window.id('notSupportedCard').style.display = "none";
            window.id('emiBinCheckError').style.display = "none";
        ...
        if(inputLength >= 6 && inputLength <= 7 && !window.alreadyPopulatedEmi && window.id("validCardCheck").style.display !== "block" && window.id("checkStartNo").style.display !== "block") {
            window.alreadyPopulatedEmi = true;
            this.emiBinCheckhandler();
        } else if (inputLength >= 9 && inputLength <= 10 && !window.alreadyPopulated && window.allowBinCheck) {            
            window.alreadyPopulated = true;
            window.tempCardBin = window.querySelector('.cardNumber').value.replace(/\s/g, '');
            this.binCheck();
        } else if(inputLength > 10 && !window.alreadyPopulated && window.allowBinCheck) {
            window.alreadyPopulated = true;
            window.tempCardBin = window.querySelector('.cardNumber').value.replace(/\s/g, '').substring(0, 9);
            this.binCheck();
        }
    
        if (window.alreadyPopulated && window.allowBinCheck) {
            this.decideBinCheck(window.querySelector('.cardNumber').value.replace(/\s/g, "").substring(0, 9));
        }
    
        let cardNumberElement = document.getElementsByClassName('pField masked')[0];
    
        if (window.allowBinCheck && this.checkFirstLetter() && this.CheckExpiryBoolean() && this.checkCvv() && this.nameCheckKeyUp() && this.checkMopTypeValidForUser() && this.checkLuhnInner(cardNumberElement) && this.checkLuhnBooleanVal()) {
            window.id('pay-now').classList.remove("btn-disabled");
        } else {
            window.id('pay-now').classList.add("btn-disabled");
        }
    ...
    emiBinCheckhandler: function() {
        let substr = window.querySelector('.cardNumber').value.replace(/\s/g, "").substring(0, 6);
        const payload = {
            'cardBin': substr,
            'payId': window.pageInfoObj.PAY_ID,
            'txnAmount': Number(window.pageInfoObj.AMOUNT) / 100,
            'bankName': window.id("emiBankName").value
        }

        fetch(`${window.basePath}/jsp/emiBinCheck`, {
            method: 'POST',
        ...
        .then(data => {
            if(data.status === 'success') {
                ...
                // window.id('notSupportedCard').style.display = "none";
                // window.id("emiBinCheckError").innerHTML = '';
            } else {
                window.allowBinCheck = false;
                window.id("emiBinCheckError").style.display = "block";
                window.id("emiBinCheckError").innerHTML = data.msg;
            }
    ...
    enterCardNum: function() {
        ...
        if(isEmiActive) {
            this.emiValidationHandler(inputLength);
        } else {
            this.cardValidationHandler(inputLength);
        }
    },
    ...
    checkErrorMsgShowOrNot: function() {
        let checkStartNoDisplay = window.id('checkStartNo').style.display,
            ...
            emiBinCheckError = window.id("emiBinCheckError").style.display,
            cardNumber = window.querySelector('.cardNumber');

        if (checkStartNoDisplay === "block" || validCardCheckDisplay === "block" || emptyCardNumberDisplay === "block" || notSupportedCardDisplay === "block" || emiBinCheckError === "block") {
    ...
    binCheck: function() {
        ...
        fetch(`${window.basePath}/jsp/binResolver`, {
            method: 'POST',
            body: JSON.stringify(payload),
            headers : {
                'Content-Type': 'application/json',
        ...
        .then(obj => {
            ...
            if(activeTabId !== undefined && activeTabId !== null) {
                ...
                if(paymentTypeUI === "EM") {
                    emiPaymentType = window.id("emiPaymentType").value;
            ...
            let ccMopIcon = document.getElementsByClassName('ccMopIcon'),
                dcMopIcon = document.getElementsByClassName('dcMopIcon'),
                ccEmiMopIcon = document.getElementsByClassName('emiccMopIcon'),
                dcEmiMopIcon = document.getElementsByClassName('emidcMopIcon');
            ...
            let allowedMopTypes = window.pageInfoObj[allowedMopObj[isEmiActive ? emiPaymentType : obj.paymentType]],
            ...
            if(obj.paymentsRegion === "DOMESTIC") {
                if (obj.mopType !== null && obj.paymentType !== null && obj.paymentType === paymentTypeUI && isValidMopType) {
                    if(isEmiActive) {
                        ...
                        if(emiBankName !== obj.issuerBankName) {
                            returnByBean = this.cardNotSupportedError({
                                ccIcon: ccMopIcon,
                                dcIcon: dcMopIcon,
                                ccEmiIcon: ccEmiMopIcon,
                                dcEmiIcon: dcEmiMopIcon,
                    ...
                    if(isEmiActive) {
                        ...
                        let _emiPaymentType = emiCardType[window.id("emiPaymentType").value];
                        ...
                        returnByBean = this.cardNotSupportedError({
                            ...
                            ccEmiIcon: ccEmiMopIcon,
                            dcEmiIcon: dcEmiMopIcon,
                            errorMsg: multilingualText(window.id("translate").value, "emiBankErrorBefore") + _emiPaymentType + multilingualText(window.id("translate").value, "emiBankErrorAfter")
                        });                        
                    } else {
                        returnByBean = this.cardNotSupportedError({
                            ccIcon: ccMopIcon,
    ...
    checkMopTypeValidForUser: function() {
        ...
        if(paymentRegion === 'DOMESTIC' || paymentRegion === 'INTERNATIONAL') {
            if(paymentType === "CC") {
                return this._checkMop(isEmiActive ? 'emiccMopIcon' : 'ccMopIcon');
            } else if(paymentType === "DC") {
                return this._checkMop(isEmiActive ? 'emidcMopIcon' : 'dcMopIcon');
            }
        } else {
            return false;
        }
    ...
    checkLuhnInner: function(element) {
        ...
        if (ipt.length === 0) {
            ...
            if (ipt.length > 12 && result) {
                ...
                if(window.id('notSupportedCard').style.display !== 'block' && window.id('emiBinCheckError').style.display !== 'block') {
