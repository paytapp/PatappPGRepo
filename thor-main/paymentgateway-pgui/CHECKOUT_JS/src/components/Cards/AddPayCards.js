...
class AddPayCards extends Component {
    state = {
        checked: true
    }
    ...
    enterCardNum = _ => {
        ...
        if (inputLength < 9) {
            ...
            Window.id("addMoneyPaymentType").value = "";
    ...
    showMopImage = data => {
        ...
        if(data.paymentsRegion === "INTERNATIONAL") {
            switch (data.paymentType) {
                case "DC":
                    this.setPaymentType({elementId: "addMoneyPaymentType", paymentType: data.paymentType, feeType: "IN"});
                break;
                case "CC":
                    this.setPaymentType({elementId: "addMoneyPaymentType", paymentType: data.paymentType, feeType: "IN"});
                break;
            ...
            switch (data.paymentType) {
                case "DC":
                    this.setPaymentType({elementId: "addMoneyPaymentType", paymentType: data.paymentType, feeType: data.paymentType});
                break;
                case "CC":
                    this.setPaymentType({elementId: "addMoneyPaymentType", paymentType: data.paymentType, feeType: data.paymentType});
                break;
    ...
    render() {
        return (            
            <React.Fragment>
                <TitleSection title={this.props.title} navigationHandler={this.props.navigationHandler} btnDataId="addAndPay" />
                ...
                <div className="container custom-container mt-170">
                    <div className="tabBox debitWithPinBox" id="debitWithPin">
                        <form autoComplete="off" name="creditcard-form" method="post" target="_self" action={`${Window.baseUrl}/pay`} id="creditCard" onSubmit={e => this.checkFields(e, "creditCard")}>
                            <div className="row cardSection">
                                ...
                                <Fade bottom duration={300}>
                                    <div className="col-lg-6 cardNumber1 card-number pb-10" id="divCardNumber">
                                        <input type="hidden" id="paymentType2" name="paymentType" value="WL" />
                                        <input type="hidden" id="addMoneyPaymentType" name="addMoneyPaymentType" />
                            ...
                            {/* <input type="hidden" name="encSessionData" value="" />
                            <input type="hidden" name="CHECKOUT_JS_FLAG" value="" /> */}

                            <input type="hidden" name="payId" value={this.props.data.PAY_ID} />
                            <input type="hidden" name="token" value={window.parent.document.getElementById("wallet-token").value} />
                            <input type="hidden" name="paymentFlow" value="ADDANDPAY" />
