...
public class AcquirerDefaultChargesAction extends AbstractSecureAction {
	...
	private static Logger logger = LoggerFactory.getLogger(AcquirerDefaultChargesAction.class.getName());
	...
	public String execute() {
		...
		try {
			...
			for (String mopdc : mopString.split(",")) {
				...
				for (CardHolderType cardType : cardHolderTypeList) {
					...
					for (String slabId : slabIdList) {
						RatesDefault ratesDef = ratesDefaultDao.findDefaultRates(acquirer, payType, mopType, acr,
								acquiringMode, slabId, industryCategoryList.get(businessType), UserType.ACQUIRER, cardType);

						if (ratesDef == null) {

							RatesDefault ratesDefault = new RatesDefault();
							ratesDefault.setAcquirer(acquirer);
							...
							ratesDefault.setAcqTdr(BigDecimal.ZERO.setScale(2));
							ratesDefault.setAcqSuf(BigDecimal.ZERO.setScale(2));
							ratesDefault.setAcquirerMaxCharge(BigDecimal.ZERO.setScale(2));
			...
			logger.error("Exception in generating list for acquirer default rates", e);
		}
	...
	public String update() {
		...
		if (StringUtils.isBlank(allDetails) || StringUtils.isBlank(acquirer)) {
			setResponse("Unable to update default rates for acquirer!");
			return SUCCESS;
		}
		...
		try {
			...
			for (String detailsRow : detailsArr) {
				...
				UserType userType = UserType.ACQUIRER;
				...
				RatesDefault rdFromDb = ratesDefaultDao.findDefaultRates(acquirer, paymentType, mopType, paymentsRegion,
				...
				if (rdFromDb == null) {
					...
					rd.setAcqTdr(new BigDecimal(detailsCells[5]));
					rd.setAcqSuf(new BigDecimal(detailsCells[6]));
					rd.setAcquirerMaxCharge(new BigDecimal(detailsCells[7]));
					rd.setAcquirer(acquirer);
					...
					try {
						...
						logger.error("Acquirer Default rates Edit Failed " , e);
						return ERROR;
					}
			...
			setResponse("Acquirer Default Rates update successfully.");
		}

		catch (Exception e) {
			logger.error("Exception while setting acquirer default rates", e);
			setResponse("Error While Updating Acquirer Default Rates");
			return ERROR;
		}
