...
import com.opensymphony.xwork2.util.WildcardUtil;
	 ...
	 */
	/**
	 * @author Apache community; just added one line to put token into session from input json
	 *
	 */
	public class CustomJsonInterceptor extends AbstractInterceptor {

		private static final long serialVersionUID = -8072117298926339915L;
		private static Logger logger = LoggerFactory.getLogger(CustomJsonInterceptor.class.getName());
	    ...
	    public String intercept(ActionInvocation invocation) throws Exception {
	        ...
	        if ((contentType != null) && contentType.equalsIgnoreCase("application/json")) {
	            ...
	            if (obj instanceof Map) {
					...
					Map json = (Map) obj;
	                session.put(TokenValidationInterceptor.FRONT_END_NAME, json.get(TokenValidationInterceptor.FRONT_END_NAME));
	                // clean up the values
	                if (dataCleaner != null)
	            ...
	            if (logger.isDebugEnabled()) {
	            	logger.debug("Content type must be 'application/json' or 'application/json-rpc'. " +
