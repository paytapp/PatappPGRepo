...
public class SettledTransactionSearchAction extends AbstractSecureAction {
	...
	private static Logger logger = LoggerFactory.getLogger(SettledTransactionSearchAction.class.getName());
	...
	public String execute() {
		...
		setStatus(StatusType.SETTLED.getName());
		...
		try {
			...
			if (sessionUser.getUserType().equals(UserType.RESELLER)) {
				totalCount = txnReports.transactionCount(getTransactionId(), getOrderId(), getCustomerEmail(),
						...
						getTransactionType(), getDateFrom(), getDateTo(), sessionUser, partSettleFlag, "ALL", null, transactionFlag,
						deltaFlag, autoRefund);
				...
				aaData = encoder.encodeTransactionSearchObj(txnReports.transactionReport(getTransactionId(),
						...
						getStart(), getLength(), partSettleFlag, "ALL", null, transactionFlag, deltaFlag, autoRefund));
				...
				totalCount = txnReports.transactionCount(getTransactionId(), getOrderId(), getCustomerEmail(),
						...
						orderIdSet, transactionFlag, deltaFlag, autoRefund);
				BigInteger bigInt = BigInteger.valueOf(totalCount);
				setRecordsTotal(bigInt);
				if (getLength() == -1) {
					setLength(getRecordsTotal().intValue());
				...
				aaData = encoder.encodeTransactionSearchObj(txnReports.transactionReport(getTransactionId(),
						getOrderId(), getCustomerEmail(), merchantPayId, subMerchantPayId, getPaymentType(),
						getStatus(), getCurrency(), getTransactionType(), getDateFrom(), getDateTo(), sessionUser,
						getStart(), getLength(), partSettleFlag, "ALL", orderIdSet, transactionFlag, deltaFlag, autoRefund));
				recordsFiltered = recordsTotal;
				...
				if (!StringUtils.isEmpty(sessionUser.getSubUserType())
					...
					aaData = encoder.encodeTransactionSearchObj(txnReports.refundForSaleCaputureTransaction(
							txnReports.transactionReport(getTransactionId(), getOrderId(), getCustomerEmail(),
									...
									getLength(), partSettleFlag, "ALL", orderIdSet, transactionFlag, deltaFlag, autoRefund)));
					recordsFiltered = recordsTotal;
					...
					totalCount = txnReports.transactionCount(getTransactionId(), getOrderId(), getCustomerEmail(),
							...
							transactionFlag, deltaFlag, autoRefund);
					BigInteger bigInt = BigInteger.valueOf(totalCount);
					setRecordsTotal(bigInt);
					if (getLength() == -1) {
						setLength(getRecordsTotal().intValue());
					}
					aaData = encoder.encodeTransactionSearchObj(
							txnReports.refundForSaleCaputureTransaction(txnReports.transactionReport(getTransactionId(),
									getOrderId(), getCustomerEmail(), merchantPayId, subMerchantPayId, getPaymentType(),
									getStatus(), getCurrency(), getTransactionType(), getDateFrom(), getDateTo(),
									sessionUser, getStart(), getLength(), partSettleFlag, "ALL", null, transactionFlag, deltaFlag, autoRefund)));
					recordsFiltered = recordsTotal;
				}
				//setRetailMerchantFlag(userSettings.isRetailMerchantFlag());
