...
public class RouterConfigurationBulkAction extends AbstractSecureAction {
	...
	private static Logger logger = LoggerFactory.getLogger(RouterConfigurationBulkAction.class.getName());
	...
	List<RouterConfiguration> aaData = new ArrayList<RouterConfiguration>();
	...
	public String execute() {
		...
		try {
			...
			routerConfigurationListAll = routerConfigurationDao.getActiveRulesByMerchant(getMerchantName(),
					getCardHolderType(), getPaymentMethod(), getAcquiringMode());
			...
			for (RouterConfiguration routerConfiguration : routerConfigurationListAll) {
				...
				String key = routerConfiguration.getPaymentType() + "-" + routerConfiguration.getMopType() + "-"
						...
						+ routerConfiguration.getCurrency() + "-" + routerConfiguration.getPaymentsRegion() + "-"
						+ routerConfiguration.getCardHolderType() + "-" + routerConfiguration.getSlabId();

				uniqueKeySet.add(key);
			}

			List<RouterConfiguration> routerConfigListNew = new ArrayList<RouterConfiguration>();
			...
			for (String uniqueKey : uniqueKeySet) {
				...
				for (RouterConfiguration routerConfiguration : routerConfigurationListAll) {
					...
					// for (RouterConfiguration router : routerConfigurationList) {
					boolean routerAdded = false;
					for (RouterConfiguration routerNew : routerConfigListNew) {
						if (routerNew.getAcquirer().equalsIgnoreCase(routerConfiguration.getAcquirer())) {
							routerAdded = true;
							break;
						}
					}
					if (!routerAdded) {
						...
						routerConfig.setAcquirer(routerConfiguration.getAcquirer());
						routerConfig.setStatusName(routerConfiguration.getStatusName());
						routerConfig.setLoadPercentage(routerConfiguration.getLoadPercentage());
						routerConfigListNew.add(routerConfig);
					}
					// }
