...
public class SaleMerchantNameAction extends AbstractSecureAction {
	...
	private boolean dispatchSlipFlag = false;
		...
		try {
			...
			if (sessionUser.getUserType().equals(UserType.ADMIN)
				...
				if (StringUtils.isNotBlank(dispatchIdentifierKey)
						&& dispatchIdentifierKey.contains(sessionUser.getPayId())) {
					setDispatchSlipFlag(true);
				}
				...
				if (sessionUser.getUserType().equals(UserType.MERCHANT) && sessionUser.isSuperMerchant() == false
					...
					merchantList.clear();
					Merchants subMerchant = new Merchants();
					subMerchant.setEmailId(sessionUser.getEmailId());
					subMerchant.setBusinessName(sessionUser.getBusinessName());
					subMerchant.setPayId(sessionUser.getPayId());
					merchantList.add(subMerchant);
	...
	public String productReport() {
		/*try {
			...
			if (sessionUser.getUserType().equals(UserType.ADMIN)
				...
				User user = userDao.findPayId(propertiesManager.propertiesMap.get("KHADI_SUPER_MERCHANT_PAYID"));
				Merchants merchant = new Merchants();
				merchant.setEmailId(user.getEmailId());
				merchant.setBusinessName(user.getBusinessName());
				merchant.setPayId(user.getPayId());
				...
				if ((propertiesManager.propertiesMap.get("KHADI_SUPER_MERCHANT_PAYID")
						.equalsIgnoreCase(sessionUser.getPayId()))
						&& (Boolean.valueOf(propertiesManager.propertiesMap.get("KHADI_SUB_USER_MODE")))) {
					setSubMerchantList(userDao
							.getActiveKhadiSubUsers(propertiesManager.propertiesMap.get("KHADI_SUPER_MERCHANT_PAYID")));
				}
