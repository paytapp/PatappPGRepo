...
function defaultData(data){ 
    ...
    if(!data.typeOfEntity) {
        ...
        else if(data.typeOfEntity == "Private Limited" || data.typeOfEntity == "Public Limited" || data.stage == "00"){
            for(key in data){
...
function checkAllField(e) {
    ...
    if(_parent.getAttribute("id") === "stage-05"){
        var _mandatory = _parent.querySelectorAll("b");
        console.log(_mandatory);
        if(_mandatory.length === 1){
...
$(document).ready(function() {
        ...
        if(registrationNumber != "" && registrationNumber != undefined && tradingState != ""){
            if(tradingState == "West Bengal" || tradingState == "Telangana" || tradingState == "Delhi" || tradingState == "Haryana" || tradingState == "Rajasthan" || tradingState == "Uttar Pradesh" || tradingState == "Maharashtra"){
                ...
                $.ajax({
                    type: "post",
                    url: "snecs",
                    data: {registrationNumber: registrationNumber, tradingState: tradingState},
                    success:function(data){
                        if(!data.mpaData.ERROR){
                            defaultData(data.mpaData);
                        }
        ...
        if (businessPan != "" && tradingState != "" && companyEmailId != "") {
            if($("#businessPan").hasClass("red-line")){
                ...
                $.ajax({
                    ...
                    url: "panToGst",
                    timeout: 180000,
                    data: { "businessPan": businessPan, "tradingState": tradingState, "companyEmailId": companyEmailId, "companyName": companyName },
                    success: function(data) {
                        if(!data.mpaData.ERROR){
    ...
    $("body").on("blur", "[data-pan='businessPanFunc']", function(e){
        ...
        if(companyName != "" && companyEmailId != "" && businessPan != ""){
            ...
            $.ajax({
                ...
                data: { companyName: companyName, companyEmailId: companyEmailId, businessPan: businessPan },
                success: function(data){
                    if(!data.mpaData.ERROR){
    ...
    $(".documentUpload").on("change", function(e){
        ...
        var _tmpFile = $(this).val();
        var _fileName = _tmpFile.replace("C:\\fakepath\\", "");
        if(_size < 2000000){
            $("#documentUpload").find(".generic-upload").removeClass("error-upload");
            $("#documentUpload").find("input").removeClass("error-sign");
            $("#documentUpload span").text(_fileName);
            $.ajax({
                type: "POST",
                enctype: 'multipart/form-data',
                url: "processImage",
            ...
            $("#documentUpload").find(".generic-upload").addClass("error-upload");
            $("#documentUpload").find("input").addClass("error-sign");
            $("#documentUpload span").text("Your File size is too long.")
        }
    })
    ...
    $("#verify-go-dl").click(function(e){
        ...
        $.ajax({
            ...
            data: {directorNumber: directorNumber},
            success: function(data){
                ...
                if(!data.mpaData.ERROR){
                    ...
                    checkAllField(e);
                    document.querySelector("#dl").setAttribute("src", '../image/uploadFile.png');
                    document.querySelector("#uploadDL").value = "";
                    $(".open-address-popup").fadeOut();
    ...
    $("#esign").on("click", function(e){
        ...
        if(_getName != ""){
            ...
            $.ajax({
                type: "post",
                url: "processESign",
                data: {"esignName": _getName},
                success: function(data){
                    console.log(data);
                    var newV = setInterval(setData, 10000);
                    windowOpen =  window.open(data.urlMap.url, "_blank", "toolbar=1,screenY="+50+", screenX="+100+" scrollbars=1, resizable=1, width=" + 800 + ", height=" + 600);
                    function setData(){
                        $.ajax({
                            type: "POST",
                            url: "esignResponseData",
                            success:function(data){
                                console.log(data);
                                if(data.mpaData != null){
                                    windowOpen.close();
                                    $("#form-body").addClass("d-none");
                                    $(".from-response-data").removeClass("d-none");
                                    $(".save-esign-data").removeClass("d-none");
                                    for(key in data.mpaData){
                                        $("[data-id="+key+"]").text(data.mpaData[key]);
                                    }
                                    $("#esign-link").attr("href", data.mpaData.esignedFile)
                                    clearInterval(newV);
                                    $("#btn-next-stage").attr("disabled", false);
                                    $(".loader-container").fadeOut();
                                    $("body").addClass("loader--inactive");
            ...
            //     var newV = setInterval(setData, 10000);
            //     windowOpen =  window.open(data.mpaData.url, "_blank", "toolbar=1,screenY="+50+", screenX="+100+" scrollbars=1, resizable=1, width=" + 800 + ", height=" + 600);
            //     function setData(){
            //         $.ajax({
            //             type: "POST",
            //             url: "getEsignResponseData",
            //             success:function(data){
            //                 if(data.mpaData != null){
            //                     windowOpen.close();
            //                     clearInterval(newV);
    ...
    $("#esignAdhar").on("change", function(e){
        var _getValue = $(this).val();
        if(_getValue.length == 12){
            $(this).removeClass("error-sign");
        }else{
            $(this).addClass("error-sign");
        }
    });

    $("#esignAdhar").on("keyup", function(e){
        $(this).removeClass("error-sign");
    });

    var _checked = false;

    $(".esign-control").on("change", function(d){
        $(".esign-control").each(function(e){
            var _this = $(this).val();
            if(_this == ""){
                _checked = false;
                return false;
            }else{
                if($(this).hasClass("error-sign")){
                    _checked = false;
                    return false;
                }else{
                    _checked = true;
       ...
       if(_checked == true){
            $("#esign").attr("disabled", false);
       }else{
            $("#esign").attr("disabled", true);
       }
    });

    $("#termCheck").on("change", function(e){
    ...
    $('#myModal').on('hidden.bs.modal', function (e) {
        if($(".from-response-data").hasClass("d-none")){
            $("#termCheck").prop("checked", false);
            $(".esign-control").val("");
            $("#documentUpload .generic-upload span").html("<b>*</b> Upload Your sign");
            $("#esign").attr("disabled", true);
        }else{
            $("#termCheck").attr("disabled", true);
        }
    });
