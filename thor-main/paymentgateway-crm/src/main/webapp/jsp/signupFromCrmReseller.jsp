...
<body>
	...
	<s:hidden id="resellerSuperMerchantSignupFlag" value="%{#session['USER'].resellerSuperMerchantSignupFlag}"></s:hidden>
	<s:hidden id="resellerSubMerchantSignupFlag" value="%{#session['USER'].resellerSubMerchantSignupFlag}"></s:hidden>
			...
			<div class="lpay_tabs_content w-100 " data-target="singleRegistration">
				...
				<s:form action="signupMerchant" id="formname" autocomplete="off">
					<div class="col-md-4 mb-20">
						...
						id="userRoleType"
						headerKey="1"
						list="#{'merchant':'Create a Merchant', 'superMerchant':'Create a Super Merchant', 'subMerchant':'Create a Sub Merchant'}" class="selectpicker">
					</s:select>					
						</div>
					</div>
					<!-- /.col-md-4 -->
					...
					<div class="col-md-4 mb-20">
						<div class="lpay_input_group pin-div">
							<label class="font-size-12 color-grey-light font-weight-medium">Confirm PIN <span class="text-danger">*</span></label>
...
<script type="text/javascript">
	...
	$(document).ready(function(e){
		...
		var _resellerMerchant = document.querySelector("#resellerMerchantSignupFlag").value;
		var _resellerSuperMerchant = document.querySelector("#resellerSuperMerchantSignupFlag").value;
		var _resellerSubMerchant = document.querySelector("#resellerSubMerchantSignupFlag").value;

		console.log(_resellerMerchant);
		console.log(_resellerSuperMerchant);
		console.log(_resellerSubMerchant)

		if(_resellerMerchant == "false"){
			$('#userRoleType').find('[value=merchant]').remove();
            $('#userRoleType').selectpicker('refresh');
		}

		if(_resellerSubMerchant == "false"){
			$('#userRoleType').find('[value=subMerchant]').remove();
            $('#userRoleType').selectpicker('refresh');
		}

		if(_resellerSuperMerchant == "false"){
			$('#userRoleType').find('[value=superMerchant]').remove();
            $('#userRoleType').selectpicker('refresh');
		}
