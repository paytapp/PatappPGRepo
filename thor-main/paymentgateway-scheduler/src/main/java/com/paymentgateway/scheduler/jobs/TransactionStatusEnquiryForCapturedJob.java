...
public class TransactionStatusEnquiryForCapturedJob extends QuartzJobBean {
	...
	private static final Logger logger = LoggerFactory.getLogger(TransactionStatusEnquiryForCapturedJob.class);
	...
	private void fetchAcquirerData() {
		...
		try {
			...
			Set<String> pgRefSet = transactionDataProvider.fetchTransactionCapturedPgRefData();
			String bankStatusEnquiryUrl = configurationProvider.getEnquiryApiUrl();

			for (String pgRef : pgRefSet) {

				logger.info("Checking Captured status enquiry request , URL = " + bankStatusEnquiryUrl + " 	pgRef == " + pgRef);
