 ...
 * @author Rajit Sharma
 */
public class UpiAutoPayDebitTransactionScheduleJob extends QuartzJobBean {
	...
	private static final Logger logger = LoggerFactory.getLogger(UpiAutoPayDebitTransactionScheduleJob.class);
	...
	private void fetchDebitTransaction() {
		...
		try {
			...
			logger.info("Debit Transaction Date = " + dueDate);
			...
			queryList.put(FieldType.STATUS.getName(), StatusType.NOTIFIED.getName());
			...
			logger.info("Query to get data for debit transactions = " + finalquery);
			...
			while (cursor.hasNext()) {
				...
				Map<String, String> res = transactionControllerServiceProvider.transact(fields,
						configurationProvider.getUpiAutoPayDebitTransactionScheduleUrl());
