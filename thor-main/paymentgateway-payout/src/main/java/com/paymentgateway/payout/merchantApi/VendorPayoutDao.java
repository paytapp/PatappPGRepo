...
public class VendorPayoutDao {
		...
		try {
			...
			while (cursor.hasNext()) {
				...
				if (StringUtils.isNotBlank(dbobj.getString(FieldType.PG_TXN_MESSAGE.getName()))) {
					...
					fields.put(FieldType.PG_TXN_MESSAGE.getName(), "NA");
				}

				if (StringUtils.isNotBlank(dbobj.getString(FieldType.RESPONSE_CODE.getName()))) {
					fields.put(FieldType.RESPONSE_CODE.getName(), dbobj.getString(FieldType.RESPONSE_CODE.getName()));
				}
	...
	public void insertUpdateForClosing(Fields fields) throws ParseException {
		...
		try {
			...
			newFieldsObj1.append(FieldType.ACQUIRER_NAME.getName(), fields.get(FieldType.ACQUIRER_NAME.getName()));
			...
			while (cursor.hasNext()) {
				...
				debitAmount = impsAmount
						.add(new BigDecimal(dbobj.getString(FieldType.DEBIT_AMOUNT.getName())).setScale(2));
			...
			if (flag) {
				...
				if (user.isSuperMerchant() == false && StringUtils.isNotBlank(user.getSuperMerchantId())) {
					filter = new Document(FieldType.SUB_MERCHANT_ID.getName(), fields.get(FieldType.PAY_ID.getName()))
							...
							.append(FieldType.ACQUIRER_NAME.getName(), fields.get(FieldType.ACQUIRER_NAME.getName()));
				} else {
				...
				if (fields.contains(FieldType.RESELLER_ID.getName())) {
					newValue = new Document(FieldType.DEBIT_AMOUNT.getName(), debitAmount.toString())
				...
				if (cursor2.hasNext()) {
					...
					while (cursor3.hasNext()) {
						...
						doc.put(FieldType.VIRTUAL_AC_CODE.getName(),
						...
						logger.info("Insert in closing collection ,virtual account code "
								+ doc.get(FieldType.VIRTUAL_AC_CODE.getName()).toString() + " , " + doc);
					}
				}
				...
				while (cursor4.hasNext()) {
					...
					logger.info("orginal data in closing collection for update ,virtual account code "
				...
				if (flag1) {
					if (closingAmount.compareTo(BigDecimal.ZERO) >= 0) {
						...
						if (user.isSuperMerchant() == false && StringUtils.isNotBlank(user.getSuperMerchantId())) {
							filter = new Document(FieldType.SUB_MERCHANT_ID.getName(),
									fields.get(FieldType.PAY_ID.getName()))
											.append(FieldType.CLOSING_DATE.getName(), dateFrom)
											.append(FieldType.ACQUIRER_NAME.getName(),
													fields.get(FieldType.ACQUIRER_NAME.getName()));
						} else {
							filter = new Document(FieldType.PAY_ID.getName(), fields.get(FieldType.PAY_ID.getName()))
									.append(FieldType.CLOSING_DATE.getName(), dateFrom)
									.append(FieldType.ACQUIRER_NAME.getName(),
						...
						if (fields.contains(FieldType.RESELLER_ID.getName())) {
							newValue = new Document(FieldType.DEBIT_AMOUNT.getName(), debitAmount.toString())
									.append(FieldType.AMOUNT.getName(), closingAmount.toString())
									.append(FieldType.UPDATE_DATE.getName(), dateNowPayout)
						...
						Bson updateOperationDocument = new Document("$set", newValue);
						collection1.updateOne(filter, updateOperationDocument);
						logger.info("Update in closing closing collection , pay Id "
								+ fields.get(FieldType.PAY_ID.getName()).toString() + " , " + newValue);
					}
				}
			}
		} catch (Exception e) {
	 ...
	 * user.getVirtualAccountNo()); if (user.isSuperMerchant() == false &&
	 ...
	 * closingAmount = dbAmount1.subtract(impsAmount); openingAmount =
	 * dbAmount1; } break; } doc1.put(FieldType.DEBIT_AMOUNT.getName(),
	 * fieldAmount); if (closingAmount.toString().equalsIgnoreCase("0.00")) {
	...
	public BigDecimal getClosingTransactionAmount(Fields fields) {
		...
		try {
			...
			query.append(FieldType.ACQUIRER_NAME.getName(), fields.get(FieldType.ACQUIRER_NAME.getName()));
	...
	public void UpdateForClosing(Fields fields) throws ParseException {
		...
		try {
			...
			if (preDateIndex.equalsIgnoreCase(todaysDateIndex)) {
				...
				newFieldsObj1.put(FieldType.ACQUIRER_NAME.getName(), fields.get(FieldType.ACQUIRER_NAME.getName()));
				...
				while (cursor.hasNext()) {
					...
					if (!flag) {
						...
						BigDecimal debitAmount = new BigDecimal(dbobj.getString(FieldType.DEBIT_AMOUNT.getName()))
								.subtract(impsAmount).setScale(2);
						closingAmount = dbAmount.add(impsAmount);
						logger.info("debitAmount : " + debitAmount);
						logger.info("closingAmount : " + closingAmount);
						Bson filter;
						...
						collection1.updateOne(filter, updateOperationDocument);
						logger.info(
								"Updated Closing collection by status enquiry, Final request : ,virtual account code "
										+ dbobj.get(FieldType.VIRTUAL_AC_CODE.getName()).toString() + " , "
										+ newValue.toString());
					} else {
						if (flag) {
							...
							dbOpeningAmount = dbOpeningAmount
									.add(new BigDecimal(fields.get(FieldType.AMOUNT.getName())));
	...
	public void checkDuplicateOrderIdMerchantInitiatedDirect(Fields fields) {
		try {
			...
			// MDC.put(FieldType.INTERNAL_CUSTOM_MDC.getName(),
			// fields.getCustomMDC());
			String message = "Error While checking duplicate order Id";
			logger.error(message, exception);
	...
	public void checkVpaDuplicateOrderId(Fields fields) {
		try {
			...
			// BasicDBObject(FieldType.CURRENCY_CODE.getName(),
			// currencyString));
			if (StringUtils.isNotBlank(subMerchantId)) {
				conditionList.add(new BasicDBObject(FieldType.SUB_MERCHANT_ID.getName(), subMerchantId));
			}
	...
	public Fields getMerchantBalance(Fields fields) {

		try {
			Date dNow = new Date();
			String dateNow = DateCreater.formatDateForDb(dNow);
			String dateIndex = dateNow.substring(0, 10).replace("-", "");

			String payId = fields.get(FieldType.PAY_ID.getName());

			List<BasicDBObject> conditionList = new ArrayList<BasicDBObject>();

			conditionList.add(new BasicDBObject(FieldType.DATE_INDEX.getName(), dateIndex));

			User user = userDao.findPayId(payId);

			if (user != null && StringUtils.isNotBlank(user.getSuperMerchantId())) {
				conditionList.add(new BasicDBObject(FieldType.SUB_MERCHANT_ID.getName(), user.getPayId()));
			} else {
				conditionList.add(new BasicDBObject(FieldType.PAY_ID.getName(), user.getPayId()));
			}
			...
			if (cursor.hasNext()) {
				...
				// Current balance
				String amount = dbobj.get(FieldType.AMOUNT.getName()).toString();
				...
				fields.put(FieldType.AVAILABLE_BALANCE.getName(), amount);
				...
				fields.put(FieldType.RESPONSE_CODE.getName(), ErrorType.SUCCESS.getCode());
				fields.put(FieldType.RESPONSE_MESSAGE.getName(), ErrorType.SUCCESS.getInternalMessage());
				fields.put(FieldType.AVAILABLE_BALANCE.getName(), "0.00");
				fields.put(FieldType.PAYOUT_DATE.getName(), dateNow);
				fields.put(FieldType.HASH.getName(), Hasher.getHash(fields));
			}
			return fields;
		} catch (Exception e) {
			logger.info("exception in fetching payout balance ", e);
		}
	...
	public boolean checkTxnFinalStatus(Fields fields) {
		logger.info("inside the checkTxnFinalStatus()");
		try {

			String txnId = fields.get(FieldType.TXN_ID.getName());

			List<BasicDBObject> conditionList = new ArrayList<BasicDBObject>();
			conditionList.add(new BasicDBObject(FieldType.TXN_ID.getName(), txnId));
			conditionList.add(new BasicDBObject(FieldType.IS_STATUS_FINAL.getName(), "Y"));
			...
			MongoCursor<Document> cursor = collection.find(query).iterator();
			if (cursor.hasNext()) {
				logger.info("transaction found for already updated status, TxnId " + txnId);
				return true;
			}
			cursor.close();
			...
			String message = "Error while checkTxnFinalStatus transaction in database";
			logger.error(message, exception);
	...
	public void getTransactionPayId(Fields fields) {

		try {
			MongoDatabase dbIns = mongoInstance.getDB();
			MongoCollection<Document> collection = dbIns.getCollection(
					propertiesManager.propertiesMap.get(prefix + Constants.IMPS_SETTlEMENT_COLLECTION.getValue()));
			BasicDBObject filterQuery = new BasicDBObject(FieldType.TXN_ID.getName(), fields.get("transferId"));
			...
			logger.info("exception in getTransactionPayId()", e);
		}
			...
			if (userSettings.isAllowECollectionFee()) {
				...
				for (SUFDetail sufCharges : sufDetailsOfMerchant) {
					...
					if (amount.compareTo(minSlab) >= 0 && amount.compareTo(maxSlab) <= 0) {
						...
						// if
						// (StringUtils.isNotBlank(sufDetails.getFixedCharge()))
						// {
						// dbFixedCharges = new
						// BigDecimal(sufDetails.getPercentageAmount());
						// BigDecimal fcGst = BigDecimal.ZERO;
						//
						// if (dbFixedCharges.compareTo(BigDecimal.ZERO) == 1) {
						//
						// calFixedCharges =
						// amount.multiply(dbFixedCharges).divide(new
						// BigDecimal("100"))
						// .setScale(2, RoundingMode.HALF_UP);
						// ;
						// fcGst = fcGst.add(
						// calFixedCharges.multiply(new
						// BigDecimal("18")).divide(new BigDecimal("100")))
						// .setScale(2, RoundingMode.HALF_UP);
						// ; // 18% GST
						// amount =
						// amount.subtract(calFixedCharges.add(fcGst)).setScale(2,
						// RoundingMode.HALF_UP);
						//
						// gst = gst.add(fcGst);
						//
						// fields.put(FieldType.FIXED_CHARGES.getName(),
						// String.valueOf(calFixedCharges));
						//
						// }
						// }
