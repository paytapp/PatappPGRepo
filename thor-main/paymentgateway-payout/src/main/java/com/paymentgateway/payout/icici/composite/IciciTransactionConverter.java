...
public class IciciTransactionConverter {
	...
	public String createRequestForTransaction(Fields fields) {
		...
		try {
			...
			json.put(Constants.IFSC, fields.get(FieldType.IFSC.getName()));
			json.put(Constants.TRANSCTION_AMOUNT, fields.get(FieldType.AMOUNT.getName()));
			json.put(Constants.CURRENCY, currency);
			json.put(Constants.TXNTYPE, TxnType);
			json.put(Constants.PAYEENAME, fields.get(FieldType.PAYEE_NAME.getName()));
		...
		try {
			if (StringUtils.isNotBlank(json)) {
				...
				if (data.has("response")) {
					...
					if (data.has("MobileAppData")) {

						if (data.get("MobileAppData") instanceof String) {
							transaction.setMobileAppData(data.getString("MobileAppData"));
						} else if (data.get("MobileAppData") instanceof JSONObject) {
							transaction.setMobileAppData(String.valueOf(data.getJSONObject("MobileAppData")));
						} else {
							transaction.setMobileAppData("");
						}
					}
				} else if (data.has("errorCode")) {
	...
	public String createCompositeIMPSRequest(Fields fields, JSONObject adfFields) {
		...
		aggrId = adfFields.getString(Constants.ADF_1);
		bcId = adfFields.getString(Constants.ADF_16);
		passCode = adfFields.getString(Constants.ADF_15);
		remitterMobileNo = adfFields.getString(Constants.ADF_17);

		String retailerCode = Constants.R_CODE;
		...
		logger.info("ICICI COMPOSITE IMPS request : " + requestJson.toString() + " txn id " + txnId);

		return requestJson.toString();
	}

	public String createCompositeIMPSStatusEnqRequest(Fields fields, JSONObject adfFields) {
		JSONObject requestJson = new JSONObject();
	...
	public String createCompositeUPIRequest(Fields fields, JSONObject adfFields) {

		String aggrId = adfFields.getString(Constants.ADF_1);
		String deviceId = adfFields.getString(Constants.ADF_19);
		String payerVa = adfFields.getString(Constants.ADF_20);
		String mobileNo = adfFields.getString(Constants.ADF_18);
		String profileId = adfFields.getString(Constants.ADF_21);
		String channelCode = adfFields.getString(Constants.ADF_22);
		String mcc = adfFields.getString(Constants.ADF_29);

		String accountProvider = adfFields.getString(Constants.ADF_23);
		String defaultAcc = adfFields.getString(Constants.ADF_24);
		String preApproved = adfFields.getString(Constants.ADF_25);
		String defaultDebit = adfFields.getString(Constants.ADF_26);
		String defaultCredit = adfFields.getString(Constants.ADF_27);
		String upiTxnType = adfFields.getString(Constants.ADF_28);
		String merchantType = adfFields.getString(Constants.ADF_30);
	...
	public String createCompositeUPIStatusEnqRequest(Fields fields, JSONObject adfFields) {
	...
	public String createCompositeNEFTRequest(Fields fields, JSONObject adfFields) {

		String aggrId = adfFields.getString(Constants.ADF_1);
		String userId = adfFields.getString(Constants.ADF_3);
		String corpId = adfFields.getString(Constants.ADF_4);
		String URN = adfFields.getString(Constants.ADF_5);

		String aggrName = adfFields.getString(Constants.ADF_2);

		String accountNo = adfFields.getString(Constants.ADF_6);
		...
		json.put(Constants.narration1, "Payment Gateway");
		...
		json.put(Constants.WORKFLOW_REQD, "Y");
	...
	public String createCompositeRTGSRequest(Fields fields, JSONObject adfFields) {
	...
	public String createCompositeIMPSRequestForAddBene(Fields fields, JSONObject adfFields) {
	...
	public String createCompositeUPIRequestForAddBene(Fields fields, JSONObject adfFields) {
	...
	public String createCompositeAccountStatementRequest(Fields fields, JSONObject adfFields) {
		...
		try {
			...
			// CONFLG will be N for 1st hit then Bank response will have
			// LASTTRID which will use in next request with CONFLG = Y
	...
	public String createStatusCheckNeftRtgs(Fields fields, JSONObject adfFields) {
		...
		logger.info("Created RTGS/Neft Transaction Request is " + json.toString());
		return json.toString();
	}
}
